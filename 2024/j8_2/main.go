...
func main() {
		...
		for _, pos := range antenna {
			for _, pos2 := range antenna {
				if pos != pos2 {
					...
					for i := 1; antiNodePos1.x >= 0 && antiNodePos1.y >= 0 && antiNodePos1.x < len(inputParsed[0]) && antiNodePos1.y < len(inputParsed); i++ {
						if _, ok := setAntiNode[antiNodePos1]; !ok {
							setAntiNode[antiNodePos1] = true
						}
						antiNodePos1 = Position{pos.x + diffX*i, pos.y + diffY*i}
						antiNodePos2 = Position{pos2.x - diffX*i, pos2.y - diffY*i}
					}
					for i := 1; antiNodePos2.x >= 0 && antiNodePos2.y >= 0 && antiNodePos2.x < len(inputParsed[0]) && antiNodePos2.y < len(inputParsed); i++ {
						if _, ok := setAntiNode[antiNodePos2]; !ok {
	...
	// Add all antenna to the antiNode set
	for _, antenna := range mapAntenna {
		for _, pos := range antenna {
			if _, ok := setAntiNode[pos]; !ok {
				setAntiNode[pos] = true
			}
		}
	}
